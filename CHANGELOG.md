...
- Reworked `--postmortem` around a **generic factual-first composer**. The doctor now narrates already-realized simulation fields (age/sex, cause/detail, PLACE, crash/substance context, suicide-reason context and seasonal timing) before optional country-specific pathway evidence. A missing Canadian pathway rule now produces a useful factual doctor summary plus a short no-additional-evidence footer instead of a dead-end `NO SUPPORTED MODELED LEAD-UP` box.
- Added `mortality_roulette_core/postmortem_summary.py` and a code-indexed ICD knowledge layer over the bundled WHO ICD-10 title catalog. Runtime genre/attribution decisions use normalized exact codes plus explicit parent inheritance; title keywords are not used for runtime classification.
- Added `datasets/who/icd10/icd10_annotations_v1.json`, deterministically generated by `tools/build_icd_annotations.py`. The initial large block covers all 872 bundled `V01-V99` transport entries with structured victim/vehicle role, collision/event and traffic/nontraffic metadata, enabling detailed transport narratives from exact V-codes while broad ranges remain deliberately broad.
- Added reusable annotation `scope` / scoped-enrichment support for country, region, sex and age applicability. Universal ICD meaning remains visible even when a geography-specific enrichment does not apply.
- Added code-based direct-alcohol annotations for the already-supported alcohol-attributed ICD families, including `K70.*` and `G31.2`. `K70` children now retain exact subtype meaning (fatty liver, hepatitis, fibrosis/sclerosis, cirrhosis, hepatic failure, unspecified alcoholic liver disease). Proxy-only alcohol weights such as the current `I63` fallback no longer create an alcohol-themed postmortem.
- Added explicit dementia/Alzheimer condition-state semantics: realized `F00/F01/F02/F03/G30` codes establish `DEMENTIA: YES`, while `F00/G30` also establish `ALZHEIMER: YES`. Population prevalence is never printed as a probability that a condition already established by the death code is present.
- Extended Finnish dementia context with the Vantaa 85+ cohort benchmark: 214/553 clinically examined residents age 85+ had dementia at baseline in 1991 (38.7%). This remains separate from Alzheimer Europe 2025 age/sex prevalence and is never averaged into it. The Finnish rule is now code-gated rather than title-keyword-gated.
- Documented the current Public Health Agency of Canada Canadian Chronic Disease Surveillance System (CCDSS) dementia data tool/methods as the preferred source family for future Canadian age-specific prevalence benchmarks; Canadian prevalence context remains geographically separate from Alzheimer Europe.
- The Finnish politician/`💸` presentation is now a true final fallback used only when the realized stack plus supported ICD/context layers cannot produce a meaningful factual postmortem.
- Added regression coverage for exact and broad transport narration, unspecified `V89.9`, code-based dementia state, `G31.2` alcohol/neurodegeneration tags, geographic scope filtering, Canadian generic fallback narration, Vantaa benchmark rendering, and the politician last-resort rule.

## v0.13.12

- Tightened Finnish postmortem relevance: Alzheimer Europe dementia prevalence context now requires a dementia/Alzheimer-relevant realized cause stack in addition to the supported 75+ age/sex band. Old age alone no longer injects dementia context into unrelated outcomes such as cerebral infarction.
- Added boozehound-aware postmortem context. When no stronger country pathway/context exists, an alcohol-sensitive realized cause can surface the active model's exposure duration, ICD cause/detail weight, baseline-to-reweighted conditional share, latency profile, and evidence/proxy basis. Explicitly alcohol-coded ICD outcomes such as `K70.*` are distinguished from non-direct alcohol-sensitive mappings such as `I63`.
- Kept the epistemic boundary explicit: alcohol-model context is not an observed individual causal chain, hospitalization history, complication sequence, or proof that alcohol caused the individual death.
- Added regression coverage for cause-gated Finnish dementia context and both direct-alcohol and non-direct alcohol-sensitive postmortem contexts.

## v0.13.11

- Moved Deathmatch postmortem narration out of each contestant's individual FINAL CARD and into a dedicated `POSTMORTEM` section immediately below the side-by-side `DEATHMATCH RESULT` table. This keeps the factual cards and comparison table compact before the longer country-specific narration.
- Tightened Deathmatch postmortem spacing: the first doctor/politician box begins immediately after the `POSTMORTEM` underline with no empty line, stacked contestant boxes have exactly one empty line between them, and the winner banner remains after the postmortem section.
- Added regression coverage for postmortem placement, ordering, and literal newline spacing.

## v0.13.10

- Added an opt-in **modeled postmortem/pathway** layer with `--postmortem`. It runs only after a realized death/cause stack and is explicitly labeled as population-level reconstruction rather than observed individual history. Unsupported country/cause combinations fail closed instead of receiving an invented clinical story.
- Added the **Canadian Death Machine** showcase flag, `--deathmachine-ca`. With no other geography/player selection it chooses national Canada and enables the doctor/postmortem layer; it can also be combined with Canadian Deathmatch `--player` specs. Mixed/non-Canadian runs use the generic `--postmortem` flag instead.
- Added `mortality_roulette_core/pathways.py` and bundled `datasets/pathways/ca_postmortem_pathway_model_v1.json`. Initial Statistics Canada Open-Licence coverage includes snowmobile/ATV fatalities, motorcycle and passenger-vehicle event classes, pedestrian reported context, unintentional fire context, fatal falls, Alzheimer's multiple-cause associations, and diabetes multiple-cause burden.
- Added a randomized UTF-8 doctor narrator (`👩‍⚕️` / `👨‍⚕️` / `🧑‍⚕️`) using its own `POST` RNG stream. Enabling postmortem output cannot perturb mortality, cause, detail, PLACE, substance/crash context, or seasonal timing; Deathmatch gives each contestant an independent postmortem stream.
- Preserved evidence semantics per rule: event marginals remain marginals, multiple-cause certificate co-recording is not promoted to temporal causation, and qualitative/denominator-mismatched evidence is rendered as a non-random summary rather than normalized into fake precision.
- Added postmortem fields to single-run CSV logging (`postmortem_rule`, `postmortem_summary`, `postmortem_model`, `postmortem_modeled`). `--runs` and qx-only `--printout` deliberately reject the per-life postmortem presentation layer.
- Documented CIHI as **local research/reference only** under Git-ignored `local-data/cihi/`; v0.13.10 ships no CIHI workbook and no CIHI-derived runtime parameter because CIHI's Terms of Use separately restrict incorporation of CIHI Materials into software applications without prior written authorization.
- Added Finnish postmortem **background context** via `datasets/pathways/fi_postmortem_context_v1.json`. If a Finnish player dies at age 75+ without a supported pathway, the doctor box can show Alzheimer Europe 2025 dementia prevalence for that exact age/sex band (men: 7.2%, 11.1%, 16.9%, 30.8%; women: 8.0%, 13.2%, 24.4%, 44.7%), explicitly labeled as population prevalence rather than an inferred diagnosis.
- Added the Finnish no-evidence arcade fallback: `🇫🇮 👨‍💼` delivers the deliberately absurd `LOOK. PEOPLE DIE... BUT WHO CARES? ... THEY DIED!` presentation followed by `💸`, with a sober line immediately below stating that no Finland-specific modeled lead-up was applied.
- Added regression coverage for pathway matching/fallbacks, Finnish age/sex context selection, broad-range false-positive prevention, CLI validation, and seeded non-perturbation.
