...
### Modeled postmortem / Canadian Death Machine

`v0.13.10` added an opt-in downstream **modeled postmortem** layer. In v0.13.11, Deathmatch moved those boxes below the final comparison table; v0.13.12 tightened relevance. `v0.13.13` makes the doctor **factual-first**: every supported country first receives a narrative assembled from already-realized simulation fields (age/sex, cause/detail, PLACE, crash/substance context and timing), then exact ICD knowledge and country-specific evidence are added where applicable. A missing specialist pathway no longer erases information the simulator already rolled. The postmortem layer never changes mortality, cause/detail selection, PLACE, timing, or any upstream RNG result.

```bash
# Generic switch; currently the evidence-backed pathway model is Canadian
python mortality_roulette.py --country ca --sex m --postmortem

# Branded Canada-specific showcase preset
python mortality_roulette.py --deathmachine-ca --sex m

# Works with Canadian Deathmatch contestants too
python mortality_roulette.py --deathmachine-ca \
  --player ca:on:m --player ca:bc:f
```

The presentation uses a randomly selected doctor emoji (`👩‍⚕️`, `👨‍⚕️`, or `🧑‍⚕️`) from its own independent downstream RNG stream. The doctor is a renderer for the model, not an assertion that an individual medical record was observed. Realized simulation facts are narrated first; evidence-backed pathway material remains explicitly labeled as modeled population context. If a Canadian pathway rule has no match, the same factual doctor summary remains and receives a short `No additional Canada-specific pathway evidence...` footer. Finland uses the same factual composer; the politician/`💸` gag is now a true last resort only when the realized stack itself cannot yield a meaningful summary.

The initial Canadian public-data model covers selected cause families where Statistics Canada exposes useful aggregate structure: snowmobile and ATV fatalities, motorcycle and passenger-vehicle fatal-event classes, pedestrian fatality context, unintentional fire context, fatal falls, Alzheimer's multiple-cause associations, and the multiple-cause burden recorded with diabetes deaths. These source products do **not** all provide the same kind of evidence. The model preserves that distinction: event-type marginals stay event-type marginals, multiple-cause co-recording is not promoted to temporal causation, and qualitative evidence summaries are not converted into fake probabilities.

`--deathmachine-ca` is a Canada-only showcase alias. If no country/player mode is otherwise supplied it selects national Canada and enables the postmortem layer. Use the generic `--postmortem` switch for mixed-country Deathmatch. `v0.13.13` also adds a code-indexed ICD annotation layer (`datasets/who/icd10/icd10_annotations_v1.json`) over the bundled WHO title catalog. Runtime genre/attribution decisions are made from normalized ICD codes and explicit parent inheritance, never title keywords. The first large annotation block covers all 872 bundled `V01–V99` transport entries, so an exact transport code can contribute victim/vehicle role, collision counterpart and traffic/nontraffic status without inventing details; broad ranges such as `V40–V79` remain broad. The same table already tags direct-alcohol families such as `K70.*` and `G31.2`, plus dementia/Alzheimer families. Optional scope metadata can restrict future annotations/enrichments by country/region/sex/age without suppressing the universal ICD meaning.

Finnish Alzheimer Europe prevalence context is code-gated to realized dementia/Alzheimer ICD families; old age alone does not inject dementia into unrelated deaths such as stroke. If the realized code itself establishes dementia (`F00/F01/F02/F03/G30`), the doctor prints `DEMENTIA: YES`; Alzheimer-specific `F00/G30` also prints `ALZHEIMER: YES`. Population prevalence remains a separately labelled benchmark, not a diagnosis probability. For Finnish players age 85+, Vantaa 85+ is shown separately as an observed oldest-old cohort benchmark (214/553, 38.7% dementia at baseline) and is never averaged with Alzheimer Europe. Proxy-only boozehound weights such as the current `I63` fallback no longer create an alcohol-themed postmortem; direct alcohol-coded outcomes (for example `K70.1` or `G31.2`) and genuinely evidence-backed alcohol enrichments can still add alcohol context. The postmortem layer is per-life presentation and is intentionally unavailable in `--runs` batch mode and qx-only `--printout` mode.

The bundled `datasets/pathways/ca_postmortem_pathway_model_v1.json` contains only project-authored parameters derived from Statistics Canada products covered by the Statistics Canada Open Licence. `datasets/pathways/fi_postmortem_context_v1.json` is a separate background-context model; its Alzheimer Europe rule stores 75+ age/sex dementia prevalence but is trigger-gated to dementia/Alzheimer-relevant realized cause stacks and never converts prevalence into a new diagnosis. Alcohol-model context is derived at runtime from the already-selected boozehound cause/detail fields and the active alcohol weight model; it adds no new mortality/cause roll. CIHI workbooks may be used locally as research/reference material under `local-data/cihi/`, but they are not runtime dependencies, are not committed, and are not redistributed in releases. See `datasets/README.md` for source-by-source provenance and limitations.
...
- [Public Health Agency of Canada — Canadian Chronic Disease Surveillance System (CCDSS) data tool](https://health-infobase.canada.ca/ccdss/data-tool/) — current Canadian surveillance source for dementia including Alzheimer disease; age-specific estimates are available nationally in five-year groups. [CCDSS methods](https://health-infobase.canada.ca/ccdss/Methods) explain case definitions, prevalence measures, age grouping and data-quality limits. Canadian dementia benchmarks are kept geographically distinct from Alzheimer Europe estimates.
- [Vantaa 85+ cohort — Hall et al. 2019](https://pmc.ncbi.nlm.nih.gov/articles/PMC6343349/) — Finnish population-based oldest-old cohort context; 214/553 clinically examined participants had dementia at baseline in 1991, with later diagnoses retained as longitudinal context rather than folded into the baseline prevalence.
